...
#error "GHTableViewRowAction must be compiled under iOS8 SDK at least"
#endif
...
@interface GHTableViewRowAction : NSObject<NSCopying>
{
...
@implementation GHTableViewRowAction
...
+ (instancetype)rowActionWithStyle:(UITableViewRowActionStyle)style title:(NSString *)title handler:(void (^)(UITableViewRowAction *, NSIndexPath *))handler {
    GHTableViewRowAction *rowAction = [[GHTableViewRowAction alloc] init];
...
+ (instancetype)rowActionWithStyle:(UITableViewRowActionStyle)style image:(UIImage *)image handler:(void (^)(UITableViewRowAction * _Nullable, NSIndexPath * _Nullable))handler {
    GHTableViewRowAction *rowAction = [self rowActionWithStyle:style title:@"holder" handler:handler];
...
__attribute__((constructor)) static void GHTableViewRowActionPatchEntry(void) {
...
#error Unsupported CPU
            ...
            if (tableViewRowActionClassLocation && !*tableViewRowActionClassLocation) {
                Class class = objc_allocateClassPair(GHTableViewRowAction.class, "UITableViewRowAction", 0);
                if (class) {
